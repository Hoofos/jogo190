...
if( isset( $_POST['Incluir'] ) ) {
	...
	$maxId = $obj_connect->maxReg( 'id_campanha', 'fr_campanhas' ) ;
	...
	if ( mysql_num_rows( $q ) ) {
		...
		$deleteSql = "delete from fr_campanhas_casos where id_campanha=" . $maxId ;
		mysql_query( $insertSql );
		
		// Depois preenchemos novamente com os novos valores
		while( $result = mysql_fetch_array( $q ) ) {
			if ( $_POST['caso_' . $result['id_caso']] == "1" ) {
				$insertSql = "INSERT INTO fr_campanhas_casos(id_campanha,id_caso) VALUES(" . $maxId . "," . $result['id_caso'] . ")" ;
				mysql_query( $insertSql );
			}
		}
	}
