...
class cls_casos {
	...
	public function listar_reg( $condicao = '' ) {
		$this->obj_connect->connect() ;
		if( $condicao == "" ) {
			$sql = "select * from fr_casos where id_caso = '$this->id_caso' " ;
		}
		else {
			$sql = "select * from fr_casos " . $condicao . "ORDER BY titulo DESC" ;
		}
		$Rs  = $this->obj_connect->RunSql( $sql ) ;
		return( $Rs ) ;
		$this->obj_connect->disconnect();
	...
	public function listar_para_combo() {
		$this->obj_connect->connect() ;	
		$sql = "select * from fr_casos ORDER BY titulo DESC" ;
	...
	function salvar() {		
		...
		if( $this->id_caso == "" || $this->id_caso == 0 ) {			
			...
			$this->sql    = "insert into fr_casos (titulo, descricao, resumo, termometro, variacao_bom, variacao_medio, variacao_ruim)" ;
			$this->sql    = $this->sql . " values ('" . str_replace("'","`",$this->titulo) . "','" . str_replace("'","`",$this->descricao) . "','" . str_replace("'","`",$this->resumo) . "','$this->termometro','$this->variacao_bom','$this->variacao_medio','$this->variacao_ruim')" ;
			$Rs           = $this->obj_connect->RunSql( $this->sql ) ;
			
			// Seleciona o id do caso cadastrado
			$maxId = $this->obj_connect->maxReg( 'id_caso', 'fr_casos' ) ;
	...
	public function excluir() {
		...
		if( $this->id_caso != 0 || $this->id_caso != "" ) {
			$sql          = "delete from fr_casos where id_caso = '$this->id_caso'" ;
			$Rs           = $this->obj_connect->RunSql( $sql ) ;
			$this->status = true ;
		}
		$this->obj_connect->disconnect() ;		
