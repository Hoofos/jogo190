...
class cls_documentos {
	...
	public function listar_reg( $condicao = '' ) {
		$this->obj_connect->connect() ;
		if( $condicao == "" ) {
			$sql = "select * from fr_documentos where id_documento = '$this->id_documento' " ;
		}
		else {
			$sql = "select * from fr_documentos " . $condicao . "ORDER BY titulo_adaptado" ;
		}
		$Rs  = $this->obj_connect->RunSql( $sql ) ;
		return( $Rs ) ;
		$this->obj_connect->disconnect();
	...
	public function listar_para_combo() {
		$this->obj_connect->connect() ;	
		$sql = "select * from fr_documentos ORDER BY nome" ;
	...
	function salvar() {		
		...
		if( $this->id_documento == "" || $this->id_documento == 0 ) {			   
			if( $this->arquivo_caminho_temporario != "" ) {
				$maxId = $this->obj_connect->maxReg( 'id_documento', 'fr_documentos' ) ;
				$maxId++ ;
				$this->nome_icone = $this->salva_arquivo( $maxId ) ;
			}	
			$this->sql    = "insert into fr_documentos (id_caso, nome, url, descricao, titulo, titulo_adaptado, palavras_chave)" ;
	...
	public function excluir() {
		...
		if( $this->id_documento != 0 || $this->id_documento != "" ) {
			...
			$sql          = "delete from fr_documentos where id_documento = '$this->id_documento'" ;
			$Rs           = $this->obj_connect->RunSql( $sql ) ;
			$this->status = true ;
		}
		$this->obj_connect->disconnect() ;		
