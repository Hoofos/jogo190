...
class cls_opcoes {
	...
	public function listar_reg( $condicao = '' ) {
		$this->obj_connect->connect() ;
		if( $condicao == "" ) {
			$sql = "select * from fr_opcoes where id_opcao = '$this->id_opcao' " ;
		}
		else {
			$sql = "select * from fr_opcoes " . $condicao . "ORDER BY titulo" ;
		}
		$Rs  = $this->obj_connect->RunSql( $sql ) ;
		return( $Rs ) ;
		$this->obj_connect->disconnect();
	...
	public function listar_para_combo() {
		$this->obj_connect->connect() ;	
		$sql = "select id_opcao,titulo from fr_opcoes ORDER BY titulo" ;
	...
	function salvar() {		
		...
		if( $this->id_opcao == "" || $this->id_opcao == 0 ) {			   
			$this->sql    = "insert into fr_opcoes (id_caso, id_situacao, id_situacao_destino, titulo, texto, aviso, tipo)" ;
			$this->sql    = $this->sql . " values ('$this->id_caso','$this->id_situacao','$this->id_situacao_destino','" . str_replace("'","`",$this->titulo) . "','" . str_replace("'","`",$this->texto) . "','" . str_replace("'","`",$this->aviso) . "','$this->tipo')" ;
		...
		// alteração
			...
			$sql = $sql . " aviso= '" . str_replace("'","`",$this->aviso) . "', " ;
	...
	public function excluir() {
		...
		if( $this->id_opcao != 0 || $this->id_opcao != "" ) {
			...
			$sql          = "delete from fr_opcoes where id_opcao = '$this->id_opcao'" ;
			$Rs           = $this->obj_connect->RunSql( $sql ) ;
			$this->status = true ;
		}
		$this->obj_connect->disconnect() ;		
