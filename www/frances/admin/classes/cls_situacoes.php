...
class cls_situacoes
{
	...
	public function listar_reg($condicao = '')	
	{
		...
		if($condicao == "")
		{
			$sql = "select * from fr_situacoes where id_situacao = '$this->id_situacao' ";
			$Rs = $this->obj_connect->RunSql($sql);
			return ($Rs);
		}
		else
		{
			$sql = "select * from fr_situacoes " . $condicao . " ORDER BY identificador";
			$Rs = $this->obj_connect->RunSql($sql);
			return ($Rs);
		}
		$this->obj_connect->disconnect();
	...
	// Método p/ listar registro para ListBox
		...
		{
			$this->obj_connect->connect();	
			$sql = "select * from fr_situacoes ORDER BY identificador";
		...
		{		
	   		...
	   		if($this->id_situacao == "" || $this->id_situacao == 0) 
	   		{
				...
				if ( $this->primeira_situacao == 'Y' ) {
					$sql = "UPDATE fr_situacoes SET primeira_situacao='N' WHERE id_caso='$this->id_caso'" ;
					$Rs = $this->obj_connect->RunSql($sql);
				}

		   		$this->sql = "insert into fr_situacoes (id_caso, identificador, descricao, primeira_situacao, situacao_final, final,problematica_boa, problematica_media, problematica_ruim)";	   
		   		$this->sql = $this->sql . " values ('$this->id_caso','$this->identificador','" . str_replace("'","`",$this->descricao) . "','$this->primeira_situacao','$this->situacao_final','" . str_replace("'","`",$this->narrativa_final) . "','" . str_replace("'","`",$this->problematica_boa) . "','" . str_replace("'","`",$this->problematica_media) . "','" . str_replace("'","`",$this->problematica_ruim) . "')";
		   		$Rs = $this->obj_connect->RunSql($this->sql);
				$this->status = true;
				
				$maxId = $this->obj_connect->maxReg( 'id_situacao', 'fr_situacoes' ) ;
			...
			else 
			{
				...
				$sql = "UPDATE fr_situacoes set ";
				$sql = $sql. " id_caso   = '$this->id_caso',";
				$sql = $sql. " identificador = '$this->identificador',";
				$sql = $sql. " descricao = '" . str_replace("'","`",$this->descricao) . "',";
				$sql = $sql. " primeira_situacao = '$this->primeira_situacao',";
				$sql = $sql. " situacao_final = '$this->situacao_final',";
				$sql = $sql. " final = '" . str_replace("'","`",$this->narrativa_final) . "',";
				$sql = $sql. " problematica_boa = '" . str_replace("'","`",$this->problematica_boa) . "',";
				$sql = $sql. " problematica_media = '" . str_replace("'","`",$this->problematica_media) . "',";
				$sql = $sql. " problematica_ruim = '" . str_replace("'","`",$this->problematica_ruim) . "'";
	...
	public function excluir()
	{
		...
		//{
			$sql = "delete from fr_situacoes where id_situacao = '$this->id_situacao'" ;
			$Rs           = $this->obj_connect->RunSql( $sql ) ;
			$this->status = true;
		//}
		$this->obj_connect->disconnect();		
