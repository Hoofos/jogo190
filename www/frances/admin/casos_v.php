...
elseif( isset( $_GET['alterar'] ) || isset( $_POST['Alterar'] ) ) {
	...
	if ( mysql_num_rows( $q ) ) {
		...
		$deleteSql = "DELETE FROM fr_indicadores_casos WHERE id_caso=" . $_SESSION['id_caso'] ;
		$obj_connect->RunSql( $deleteSql );
		
		while( $result = mysql_fetch_array( $q ) ) {
			$nivel_indicador = $_POST['indicador_' . $result['id_indicador']] ;
			
			$insertSql = "INSERT INTO fr_indicadores_casos(id_indicador,id_caso,nivel_indicador) VALUES(" . $result['id_indicador'] . "," . $_SESSION['id_caso'] . "," . $nivel_indicador . ")" ;
								...
								<?php
									...
									if ( mysql_num_rows( $q ) ) {
										while( $result = mysql_fetch_array( $q ) ) {
											...
											$selectSql = "SELECT nivel_indicador FROM fr_indicadores_casos WHERE id_caso=" . $_SESSION['id_caso'] . " AND id_indicador=" . $result['id_indicador'] ;
