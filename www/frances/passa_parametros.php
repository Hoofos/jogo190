<?php
	...
	$link = mysql_connect( "mysql01.brjcomunicacao.com.br", "brjcomunicacao", "mysqlbrj01" ) or die('Could not connect: ' . mysql_error());

	// Seleciona a Base de Dados
	mysql_select_db('brjcomunicacao') or die('Could not select database');
	...
	elseif ( $tipo == 1 ) {
		...
		if ( $campanha == 1 ) {
			$query 	= 	"SELECT c.titulo,c.descricao,c.resumo,cp.termometro,cp.variacao_bom,cp.variacao_medio,cp.variacao_ruim, b.* "
					.	"FROM fr_casos AS c INNER JOIN fr_boxes AS b "
	...
	elseif( $tipo == 2 ) {
		...
		if ( $opcao == 0 ) {
			...
			$query = "SELECT s.*,o.* "
			         . "FROM fr_situacoes s INNER JOIN fr_opcoes o ON s.id_situacao=o.id_situacao "
	...
	elseif( $tipo == 3 ) {
		//
		$query = "SELECT * FROM fr_dicas WHERE id_situacao=" . $situacao . " ORDER BY titulo LIMIT 0,4" ;
		//
		$rs = mysql_query( $query ) ;
		//
		if( mysql_num_rows($rs) ) {
	...
	elseif( $tipo == 4 ) {
		//
		$query = "SELECT * FROM fr_documentos WHERE id_caso=" . $caso . " ORDER BY titulo_adaptado" ;
		$rs = mysql_query( $query ) ;
		//
		if ( mysql_num_rows($rs) ) {
			//
	...
	elseif( $tipo == 5 ) {
		//
		$query = "SELECT * FROM fr_documentos WHERE ( nome LIKE '%" . $busca . "%' ) OR "
		       . "( url LIKE '%" . $busca . "%') OR "
	...
	elseif( $tipo == 6 ) {
	//
		$query = "SELECT palavras_chave FROM fr_documentos ORDER BY RAND()" ;
