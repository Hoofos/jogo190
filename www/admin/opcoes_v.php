...
elseif( isset( $_GET['alterar'] ) || isset( $_POST['Alterar'] ) ) {
	...
	if ( mysql_num_rows( $q ) ) {
		...
		$deleteSql = "DELETE FROM vr_indicadores_opcoes WHERE id_opcao=" . $_SESSION['id_opcao'] ;
		$obj_connect->RunSql( $deleteSql );
		
		while( $result = mysql_fetch_array( $q ) ) {
			$nivel_indicador = $_POST['indicador_' . $result['id_indicador']] ;
			
			$insertSql = "INSERT INTO vr_indicadores_opcoes(id_indicador,id_opcao,nivel_indicador) VALUES(" . $result['id_indicador'] . "," . $_SESSION['id_opcao'] . "," . $nivel_indicador . ")" ;
								...
								<?php
									...
									if ( mysql_num_rows( $q ) ) {
										while( $result = mysql_fetch_array( $q ) ) {
											...
											$variacoesSQL = "SELECT nivel_indicador FROM vr_indicadores_opcoes WHERE id_opcao=" . $_SESSION['id_opcao'] . " AND id_indicador=" . $result['id_indicador'] ;
