...
class cls_campanhas {
	...
	public function listar_reg( $concampanhao = '' ) {
		$this->obj_connect->connect() ;
		if( $concampanhao == "" ) {
			$sql = "select * from vr_campanhas where id_campanha = '$this->id_campanha' " ;
		}
		else {
			$sql = "select * from vr_campanhas " . $concampanhao . "ORDER BY titulo DESC" ;
		}
		$Rs  = $this->obj_connect->RunSql( $sql ) ;
		return( $Rs ) ;
		$this->obj_connect->disconnect();
	...
	public function listar_para_combo() {
		$this->obj_connect->connect() ;	
		$sql = "select * from vr_campanhas ORDER BY titulo DESC" ;
	...
	function salvar() {		
		...
		if( $this->id_campanha == "" || $this->id_campanha == 0 ) {			   
			...
			$this->sql    = "insert into vr_campanhas (titulo, descricao)" ;
	...
	public function excluir() {
		...
		if( $this->id_campanha != 0 || $this->id_campanha != "" ) {
			...
			$sql          = "delete from vr_campanhas where id_campanha = '$this->id_campanha'" ;
			$Rs           = $this->obj_connect->RunSql( $sql ) ;
			$sql          = "delete from vr_campanhas_casos where id_campanha = '$this->id_campanha'" ;
			$Rs           = $this->obj_connect->RunSql( $sql ) ;
			$this->status = true ;
		}
		$this->obj_connect->disconnect() ;		
