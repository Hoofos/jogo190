...
class cls_boxes {
	...
	public function listar_reg( $conboxo = '' ) {
		$this->obj_connect->connect() ;
		if( $conboxo == "" ) {
			$sql = "select * from vr_boxes where id_box = '$this->id_box' " ;
		}
		else {
			$sql = "select * from vr_boxes " . $conboxo . " ORDER BY id_box" ;
		}
		$Rs  = $this->obj_connect->RunSql( $sql ) ;
		return( $Rs ) ;
		$this->obj_connect->disconnect();
	...
	public function listar_para_combo() {
		$this->obj_connect->connect() ;	
		$sql = "select * from vr_boxes ORDER BY id_box" ;
	...
	function salvar() {		
		...
		if( $this->id_box == "" || $this->id_box == 0 ) {			   
			$this->sql    = "insert into vr_boxes (id_caso, titulo_1, fontes_1, conteudo_1, titulo_2, fontes_2, conteudo_2)" ;
			$this->sql    = $this->sql . " values ('$this->id_caso','$this->titulo_1','$this->fontes_1','$this->conteudo_1','$this->titulo_2','$this->fontes_2','$this->conteudo_2')" ;
			$Rs           = $this->obj_connect->RunSql( $this->sql ) ;
			$this->status = true ;	 						   	  		
		}  
		// alteraзгo
		else {
			$sql = "UPDATE vr_boxes set " ;
			$sql = $sql . " id_caso   = '$this->id_caso', " ;
			$sql = $sql . " titulo_1 = '$this->titulo_1', " ;
			$sql = $sql . " fontes_1  = '$this->fontes_1', " ;
			$sql = $sql . " conteudo_1 = '$this->conteudo_1', " ;
			$sql = $sql . " titulo_2  = '$this->titulo_2', " ;
			$sql = $sql . " fontes_2 = '$this->fontes_2', " ;
			$sql = $sql . " conteudo_2  = '$this->conteudo_2' " ;
	...
	public function excluir() {
		...
		if( $this->id_box != 0 || $this->id_box != "" ) {
			$sql          = "delete from vr_boxes where id_box = '$this->id_box'" ;
			$Rs           = $this->obj_connect->RunSql( $sql ) ;
			$this->status = true ;
		}
		$this->obj_connect->disconnect() ;		
