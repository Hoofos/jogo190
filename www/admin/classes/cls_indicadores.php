...
class cls_indicadores {
	...
	public function listar_tudo() {
		...
		$sql = "select * from vr_indicadores" ;
	...
	public function listar_reg( $condicao = '' ) {
		$this->obj_connect->connect() ;
		if( $condicao == "" ) {
			$sql = "select * from vr_indicadores where id_indicador = '$this->id_indicador' " ;
		}
		else {
			$sql = "select * from vr_indicadores " . $condicao . "ORDER BY nome" ;
		}
		$Rs  = $this->obj_connect->RunSql( $sql ) ;
		return( $Rs ) ;
		$this->obj_connect->disconnect();
	...
	public function listar_para_combo() {
		$this->obj_connect->connect() ;	
		$sql = "select id_indicador,nome from vr_indicadores ORDER BY nome" ;
	...
	function salvar() {		
		...
		if( $this->id_indicador == "" || $this->id_indicador == 0 ) {			   
			if( $this->icone_caminho_temporario != "" ) {
				$maxId = $this->obj_connect->maxReg( 'id_indicador', 'vr_indicadores' ) ;
				$maxId++ ;
				$this->nome_icone = $this->salva_img( $maxId ) ;
			}	
			$this->sql    = "insert into vr_indicadores (nome, descricao, nivel_max, nivel_min)" ;
	...
	public function excluir() {
		...
		if( $this->id_indicador != 0 || $this->id_indicador != "" ) {
			...
			$sql          = "delete from vr_indicadores where id_indicador = '$this->id_indicador'" ;
			$Rs           = $this->obj_connect->RunSql( $sql ) ;
			$this->status = true ;
		}
		$this->obj_connect->disconnect() ;		
