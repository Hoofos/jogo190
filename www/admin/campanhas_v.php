...
elseif( isset( $_GET['alterar'] ) || isset( $_POST['Alterar'] ) ) {
	...
	if ( mysql_num_rows( $q ) ) {
		...
		$deleteSql = "delete from vr_campanhas_casos where id_campanha=" . $_SESSION['id_campanha'] ;
		$obj_connect->RunSql( $deleteSql );
		
		// Depois preenchemos novamente com os novos valores
		while( $result = mysql_fetch_array( $q ) ) {
			if ( isset($_POST['caso_' . $result['id_caso']]) ) {
				$insertSql = "INSERT INTO vr_campanhas_casos(id_campanha,id_caso) VALUES(" . $_SESSION['id_campanha'] . "," . $result['id_caso'] . ")" ;
							...
							<?php
								...
								if ( mysql_num_rows( $q ) ) {
									while( $result = mysql_fetch_array( $q ) ) {
										...
										$selectSql = "SELECT * FROM vr_campanhas_casos WHERE id_caso=" . $result['id_caso'] . " AND id_campanha=" . $_SESSION['id_campanha'] ;
